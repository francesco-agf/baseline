...
| file | cos'è |
|---|---|
| `sorgente/baseline.html` | il **sorgente di lavoro**. Non ha doctype né `<head>`: quando lo si pubblica come artifact su claude.ai è la cornice a metterli. È qui che si modifica. |
...
```sh
node prova-regressione.js       # le meccaniche di base
node prova-segni-e-barra.js    # segni speciali, barra colore, suoni, pausa, invito
node prova-anteprima.js        # l'anteprima coincide col pezzo che scende, per 140 pezzi
node prova-regole.js           # i numeri della pagina delle regole contro il codice
node prova-abbandono.js        # il pulsante «abbandona» in pausa
node prova-di-stampa.js        # il JPEG e la GIF di fine partita
node prova-di-stampa-limiti.js # gli stessi, senza righe stampate e con partita lunga
node prova-classifica.js       # invio del punteggio con il database finto
